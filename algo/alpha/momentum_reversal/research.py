...
def get_dfst_trading(dfst_feature, trading_param: MomentumReversalTradingParam):
    if 'rank' not in dfst_feature.columns:
        ...
        symbol_with_momentums = dfst_feature[
            ((dfst_feature['rank'] <= trading_param.selection_size) & (dfst_feature['momentum'] < 0))
            | ((dfst_feature['rank_descending'] <= trading_param.selection_size) & (dfst_feature['momentum'] > 0))

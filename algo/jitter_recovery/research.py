...
def get_dfsts_feature(df, trading_param):
    dfi = df.set_index(['timestamp', 'symbol'])
    all_symbols = df.symbol.unique()
    all_symbols = [s for s in all_symbols if 'USDT' in s]
    ...
    return dfst_feature_approximate, dfst_feature


def get_dfst_trading(df, dfst_feature_approximate, trading_param):
    dfi = df.set_index(['timestamp', 'symbol'])
    dfst_with_jumps = dfst_feature_approximate[dfst_feature_approximate.ch_max >= trading_param.jump_threshold * 0.9]
    symbol_with_jumps = dfst_with_jumps.reset_index().symbol.unique()
    symbol_with_jumps = [s for s in symbol_with_jumps if 'USDT' in s]
...
def investigate_trading(dfst_feature_approximate, dfst_trading):
    fig, (ax_profit, ax_ch, ax_ch_max, ax_ch_min) = plt.subplots(4, figsize=(16,8))
